...
export function InvoiceFormDialog({ isOpen, onClose, onSuccess }: {
  ...
  const handleSubmit = async () => {
    ...
    if (createNewJob && newJobTitle.trim()) {
      const { data: newJob, error } = await supabase.from('jobs').insert({ title: newJobTitle }).select().single();
      if (error) {
    ...
    const { data: invoiceData, error: invoiceError } = await supabase
      .from('invoices')
      .insert([{ job_id: jobId, due_date, amount: totalAmount, paid: false }])
      .select()
      .single();
      ...
      <DialogContent className="max-w-3xl">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            {!createNewJob ? (
              <select className="w-full border rounded p-2" value={selectedJobId} onChange={(e) => setSelectedJobId(e.target.value)}>
              ...
              <Input placeholder="New Job Title" value={newJobTitle} onChange={(e) => setNewJobTitle(e.target.value)} />
            )}
